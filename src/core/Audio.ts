  ...
  private playRouted(name: SoundName): void {
    ...
    switch (name) {
      ...
      case 'trap_empty':
        ...
        break;
      case 'trap_spikes':
        // 가시가 솟는다 — 크되 둔탁하게: 고음 파열·쇳소리는 총소리로 들려서 뺐다 (2026-09-02).
        // 굵은 저음 둥 + 쇠가 돌 홈을 박차는 낮은 텅(200→90) + 저역만 남긴 돌 갈림 + 뒤따르는 무거운 울림
        this.tone(58, 0.32, 'sine', 1.1, 0, 40);
        this.tone(210, 0.16, 'triangle', 0.9, 0, 90);
        this.noise(0.16, 1.1, 480);
        this.tone(120, 0.26, 'triangle', 0.55, 0.05, 70);
        this.noise(0.12, 0.6, 320, 0.1);
        this.tone(46, 0.3, 'sine', 0.8, 0.12, 36);
        break;
      case 'trap_net':

  ...
  private playRouted(name: SoundName): void {
    ...
    switch (name) {
      ...
      case 'leech_suck':
        ...
        this.noise(0.28, 0.5, 900);
        this.tone(340, 0.3, 'sawtooth', 0.3, 0, 90);
        break;
      case 'leech_drip':
      ...
      case 'bone_rattle':
        ...
        break;
      case 'armor_lock':
        // 슈퍼아머 시작 — 뼈가 맞물려 잠기는 금속성 딸깍 두 번(0·45ms) + 짧은 쇳소리 꼬리. 예고음 밑에 깔려도 또렷한 고음.
        // 매번 같은 소리라 "이 소리 = 끊을 수 없다"로 학습된다
        this.tone(2400, 0.03, 'square', 0.55, 0, 1900);
        this.noise(0.02, 0.9, 6000);
        this.tone(1800, 0.04, 'square', 0.5, 0.045, 1200);
        this.noise(0.03, 0.7, 5200, 0.045);
        this.tone(620, 0.12, 'triangle', 0.35, 0.05, 520);
        break;
      case 'skeleton_death':
      ...
      case 'trap_click':
        ...
        // 1초 뒤 가시가 솟는다 — 이 소리가 "지금 뛰어라" 신호다 (걸쇠·돌 갈림은 회수 때 난다)
        this.tone(70, 0.3, 'sine', 1.0, 0, 42);
        this.noise(0.08, 0.6, 520);
        this.tone(54, 0.22, 'sine', 0.75, 0.13, 38);
      ...
      case 'trap_empty':
        ...
        break;
      case 'trap_spikes':
        // 가시가 솟는다 — 날카롭고 크게 (2026-09-02 최종): 쇠 칼날이 홈을 박차며 '챙' 하고 길게 우는 금속음.
        // 총소리(짧은 광대역 파열+저음 붐)와 갈리게 짧은 쨍 뒤 고음 금속 울림이 0.5초 이어진다
        this.noise(0.05, 1.2, 6500);
        this.tone(2600, 0.5, 'triangle', 0.9, 0, 1500);
        this.tone(3400, 0.42, 'sine', 0.7, 0.01, 2100);
        this.tone(640, 0.22, 'sawtooth', 0.9, 0, 300);
        this.noise(0.1, 0.9, 2200, 0.02);
        this.tone(70, 0.22, 'sine', 0.7, 0.03, 48);
        break;
      case 'trap_net':
      ...
      case 'trap_rumble':
        ...
        break;
      case 'trap_whoosh':
        // 칼날이 지나간다 — 공기를 가르는 스윕. 작다는 피드백에 크게 (2026-09-03)
        this.noise(0.34, 1.3, 700);
        this.noise(0.2, 0.8, 1600, 0.06);
        this.tone(320, 0.3, 'sine', 0.55, 0, 900);
        this.tone(120, 0.3, 'triangle', 0.4, 0, 70);
        break;
      case 'trap_creak':
        // 진폭 끝 — 무거운 쇠사슬이 방향을 바꾸며 삐걱인다 (낮은 긁힘 + 높은 삐이 + 사슬 짤랑). 크게
        this.tone(180, 0.42, 'sawtooth', 0.6, 0, 135);
        this.tone(1250, 0.28, 'triangle', 0.38, 0.03, 880);
        this.noise(0.32, 0.55, 620);
        this.noise(0.06, 0.7, 4200, 0.28);
        this.tone(90, 0.3, 'sine', 0.45, 0.02, 70);
        break;
      case 'trap_ping':

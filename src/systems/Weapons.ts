...
function drawBow(world: World): void {
  ...
  // 패드는 조준(LT)을 놓는 것이 곧 "쏘지 않는다" — RT 를 쥔 채 LT 만 놓아도 발사되지 않는다
  // (2026-09-02 사용자 지시). 조준을 놓으면 rangedHeld 도 꺼져 아래 '놓음' 분기가 쏴 버리므로
  // 그 전에 잡는다
  if (draw > 0 && (world.input.reload || world.input.aimReleased)) {
    w.bowDraw = 0;

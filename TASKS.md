...
- [ ] **B3-5** 아레나 f4 + `Arena.ts` — 잔해(붕괴 기둥)는 기획서 §9.3 표대로 '충돌이 아니다'(B2-5 규칙: `Level.blockedAhead` 가 벽으로 읽지 않는 props — 질주가 계속되어 시간이 다하면 헛돌격 90). 잔해 rect 를 별도 목록으로 인식해 헛돌격 60 을 내는 옛 행은 폐기(B3-4 문서 정리)

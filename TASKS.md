...
- [x] **9.1** ② 그물 덫(해머·총 해체) + ③ 기름 웅덩이(불로 점화·연쇄) + ⑥ 저주 문양(오염 +6 / 적 경직→처형) — f1 그물·기름, f2 문양 배치
- [ ] **9.2** ⑤ 독가스 배기구 + ⑦ 낙석(잔해 저지대·적 경로 차단) + ⑧ 진자 칼날(완벽 패링) — f2 가스, f3 낙석·진자 배치
- [ ] **9.3** 함정 감지 각인(`sig_trapsense`) 실구현 + 층별 배치 총점검 + `docs/systems/traps.md`

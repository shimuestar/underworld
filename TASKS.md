...
B3-5(2026-09-06)가 아레나·돌격·지형·문서 항목을 처리했다([x] 표시와 사유). 남은 [ ] 는 B3-6(보상·마무리)에서 — 관절 원뿔 데이터(B2-1), 파열 소리 중복(B2-3), 노출 중 전환(B2-6), head_down·exhaust 관절 메시 0.44(B3-4).
...
B3-5(2026-09-06)가 문서 드리프트(B3-1)·돌격 장부(B3-4 chainLeg)를 처리했다. 남은 [ ] 는 B3-6 에서 — 분출공 판정=그림(B3-2), 오염 대기 음수 표시(B3-2), 분출공 파열 규약(B3-2), 반사 구슬 웅덩이(B3-2), heavy 목록 주석(B3-3), plates 디버그 카메라(B3-3), head_down·exhaust 관절 메시(B3-4).

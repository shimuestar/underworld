...
- [ ] **10.2** 보물상자를 컨테이너로(`chestItems` 1회 롤, 각인 줄 → Sigils 습득), 부활 시 상자 재스폰 제거
- [ ] **10.3** 연출 마무리(아이콘 비행·거부 흔들림 검수) + 주머니 안착 소리 + 문서(`docs/systems/loot.md`, metrics)
- [ ] **10.4** 바닥 소모품 E 줍기(`itemInView`) + 소모품 자석 폐지 + 가득 참 튕김(`pickup_bounced`) + economy 문서 갱신

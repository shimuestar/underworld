...
2. ~~**제단은 잔탄과 무관하게 상한까지 채운다.**~~ **폐지 (2026-08).**
   제단은 이제 **골드 상점**이다. 무료 보급과 공격성 보너스 배율을 모두 없앴고,
   체력·마나·탄약·수류탄·배터리를 전부 `data/balance.json`의 `altar.shop` 가격으로 산다.
   그 결과 **탄약을 아끼면 이득**이 된다 — 원래 의도의 정반대이므로 되살릴 때 주의할 것.
   되살리려면 git 이력의 `Altar.aggressionMultiplier` 와 `enter()` 의 상한 SET 블록을 참조.

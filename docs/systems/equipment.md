# 장비 — 투구·갑옷·부츠·반지 2·목걸이·안전주머니(옛 짐칸) (2026-09-04)
...
| `bagSlots` | `Inventory.bagSizeFor` — 기본 `items.cols×rows`(5×2=10) + 안전주머니(pack) |

## 안전주머니(pack) 규칙 (벨트 ↔ 가방, 하나만) — 2026-09-08 사용자: 안전 주머니와 같은 개념이라 이름을 맞췄다. 캐릭터 아래 칸, 그 밑에 안전 주머니 속 칸(stash.md §1)
...
- **몸 패널** 이 가방 격자 왼쪽에 있다: 가운데 몸 실루엣(각인 소켓 5 — 눈·심장·왼팔·오른팔·척추, `render/BodyDoll`), 둘레에 장비 칸 7(왼쪽 투구·갑옷·부츠, 오른쪽 목걸이·반지 1·반지 2, 아래 안전주머니). 걸친 것은 아이콘·이름, 빈 칸은 부위 이름. 안전주머니 칸 아래에 안전 주머니 속 칸(죽어도 남는 것)이 붙는다.

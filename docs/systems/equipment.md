# 장비 — 투구·갑옷·부츠·반지 2·목걸이·가방(옛 짐칸, 벨트·가방만) (2026-09-04)
...
| `bagSlots` | `Inventory.bagSizeFor` — 기본 `items.cols×rows`(5×2=10) + 가방 칸(pack) 장비 |

## 가방 칸(pack) 규칙 (벨트 ↔ 가방, 하나만) — 부위 이름은 '가방'(2026-09-08 사용자, 옛 짐칸). 캐릭터 아래 칸, 그 밑에 안전 주머니 2칸이 따로 있다(stash.md §1)
...
- **몸 패널** 이 가방 격자 왼쪽에 있다: 가운데 몸 실루엣(각인 소켓 5 — 눈·심장·왼팔·오른팔·척추, `render/BodyDoll`), 둘레에 장비 칸 7(왼쪽 투구·갑옷·부츠, 오른쪽 목걸이·반지 1·반지 2, 아래 가방(벨트·가방)). 걸친 것은 아이콘·이름, 빈 칸은 부위 이름. 가방 칸 아래에 안전 주머니 칸(죽어도 남는 것, 1 열림·1 잠김)이 붙는다.
- 각인 소켓: 가방의 패시브 각인을 Y/E·A 로 새기거나 소켓에 끌어 놓는다(부위가 맞아야). 제단 앞에서 소켓 Y/E·A·우클릭 = 떼기 → 가방. (2026-09-04: 스킬 탭에서 옮겨 왔다)

...
| `World.EnemyState` | `weakHp{id → hp}`, `exposure{id → ticks}`, `exposureHits`, `weakAccum`, `dazeCooldown`, `dazed`, `pose`/`poseTicks`, `bladeLock{r?, l?}`, `ruptured{id → true}`(파열 처리 표식 — Enemies 는 지우지 않고 **갑각 재생이 `weakHp` 회복과 함께 지운다**), `limping`, `blind`(눈멂 플래그 — 질주 중만 참, 질주가 끊기면 안전망이 내린다), `chargeStuck`(질주 막힘 연속 틱 — `chargeStuckTicks` 에 닿으면 지형 충돌 판정) |

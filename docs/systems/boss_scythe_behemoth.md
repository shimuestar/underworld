...
| `World` | `pools: PoolState[]{id, x, z, r, ticks, duration, kind}` — 진액 웅덩이(Hazards 소유), `PlayerState.corrosiveTicks/corrosiveAccum`, `ProjectileState.deflectSelfDamage/poolKind/statusOnHit/statusOnBlock/playerKnockback`, kind `'goo'` — B3-2, `PlayerState.cowedTicks`(위압)·`iframeSource 'dodge'|'blink'|'escape'`(무적 출처 — 완벽 회피는 dodge 만) — B3-4 |

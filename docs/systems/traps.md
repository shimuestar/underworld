...
| `trap_gas_auto` (포자 군락) | 中 | 없음(자동 순환) | 30틱 부풀며 떨림·개화음 | 포자 구름 3.5초(독 상태) | 경둔화 0.7 | 쉼 4.5초 → 예고 0.5초 → 구름 3.5초 반복 (걷힌 뒤 5초 만에 다시) | 걷힌 5초 틈에 지나가기 |
...
- **포자 식물** (2026-09-02 재설계) — 배기구 대신 식물. 근접하면 **1초 바르르 떨며** 개화 뒤 분출, **한 번 터지면 끝**(시든 주머니·늘어진
  ...
  맞히거나, 해머 부채꼴에 들거나, 폭발(수류탄·화염구·폭발통) 반경 안이면 `provokeTrap`(World 헬퍼)으로 곧장 예고 — 멀리서

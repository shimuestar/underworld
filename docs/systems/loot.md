...
각인도 가방 아이템 — 소모품처럼 바라보며 E 로 집는다(`Pickups`, 2026-09-04 아이템화). 스킬 탭에서 새긴다. 항아리(기믹) 전리품은 1개가 튕겨 나오는 그대로.
...
- **가져오기** — 골드는 항상 전부(카운터, `gold_picked` 는 컨테이너 자리에서 ◆ 팝). 화살은 화살통 상한까지 부분(남은 건 줄에 남는다).
  소모품은 가방에 자리가 있을 때만 1개씩. 각인은 빈 칸이 있을 때 가방으로(한 칸에 한 개) — 스킬 탭에서 새긴다. 가방 → 주머니로 넣을 수도 있다(sigilId 줄).
...
- **툴팁** — 창 아래 한 줄: 소모품은 효과·지금 쓸 값어치·가방 수, 골드·화살은 쓰는 곳·소지량, 각인은 설명·부위(또는 액티브)·오염과 지금 새길 수 있는지.
...
| **사용**(퀵슬롯에 없어도, 창이 닫히고 시전 시작; 가득·쿨다운이면 거부 안내) | Y | E·더블클릭 |
| 바닥에 버리기(퀵슬롯 칸이면 등록 해제) | X | X·우클릭 |
| 수량 나누기 | X 길게 | Shift+Enter·Shift+클릭 |
| 보관 주머니 내려놓기 | Y 길게(`loot.ui.padPouchHoldTicks`) | P |
